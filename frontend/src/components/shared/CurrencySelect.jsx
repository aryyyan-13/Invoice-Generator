...
export default function CurrencySelect({ value, onChange, placeholder = 'Select currency…', style }) {
  ...
  const select = (c) => { onChange(c.code); setQuery(`${c.code} – ${c.name}`); setOpen(false); };
  const displayValue = open ? query : (value ? `${value} – ${currencies.find(c => c.code === value)?.name || ''}` : '');
    ...
    <div style={{ position: 'relative', ...style }}>
      ...
      {open && filtered.length > 0 && (
        <ul style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 1000, background: '#1e2535', border: '1px solid var(--border-color)', borderRadius: '8px', maxHeight: '200px', overflowY: 'auto', margin: 0, padding: 0, listStyle: 'none' }}>
          {filtered.map(c => (
            <li key={c.code} onMouseDown={() => select(c)} style={{ padding: '8px 14px', cursor: 'pointer', color: '#e2e8f0', fontSize: '13px' }}
